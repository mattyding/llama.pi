/* inference for quantized 4-byte llama2
 * based off of karpathy's llama.c implementation
 *
 * modifications for bare metal include:
 * - replace C stdlib fns with our r/pi implementations
 * - replace memory free's (reboot instead)
*/
#include <ctype.h>
#include <stdint.h>
#include <math.h>
...
void read_checkpoint(char* checkpoint, Config* config, TransformerWeights* weights,
    ...
    if (!file) { panic("Couldn't open file %s\n", checkpoint); }
    // read in magic number (uint32), has to be 0x616b3432, i.e. "ak42" in ASCII
    uint32_t magic_number;
    if (fread(&magic_number, sizeof(uint32_t), 1, file) != 1) { panic(); }
    if (magic_number != 0x616b3432) { panic("Bad magic number\n"); }
    // read in the version number (uint32), has to be 2
    int version;
    if (fread(&version, sizeof(int), 1, file) != 1) { panic(); }
    if (version != 2) { panic("Bad version %d, need version 2\n", version); }
    int header_size = 256; // the header size for version 2 in bytes
    // read in the Config
    if (fread(config, sizeof(Config), 1, file) != 1) { panic(); }
    // read in flags
    uint8_t shared_classifier; // a byte to indicate if the classifier is shared
    if (fread(&shared_classifier, sizeof(uint8_t), 1, file) != 1) { panic(); }
    int group_size; // the group size used in quantization
    if (fread(&group_size, sizeof(int), 1, file) != 1) { panic(); }
...
void chat(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler,
    ...
    int num_prompt_tokens = 0;
    int* prompt_tokens = (int*)kmalloc(1152 * sizeof(int));
    int user_idx;
...
void notmain(void) {
    ...
    if (strcmp(mode, "generate") == 0) {
        ...
        panic("unknown mode. modify params in params.h\n");
    }

    // restart pi to free memory
    clean_reboot();
}
#endif
