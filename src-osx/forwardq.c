...
//
void malloc_run_state(qRunState* s, Config* p) {
...
void free_run_state(qRunState* s) {
    free(s->x);
    free(s->xb);
    free(s->xb2);
    free(s->hb);
...
int  generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, char *prompt, int steps) {
    ...
    while (pos < steps) {
        ...
        if (start == 0) { start = time_in_ms(); start_timer(); }
    }
    printf("\n");
    ...
    if (pos == 0) {
        ...
        stop_timer();
        long end = time_in_ms();
        fprintf(stderr, "vanil: achieved tok/s: %f\n", (pos-1) / (double)(end-start)*1000);
    }
...
int main(int argc, char *argv[]) {
    ...
    // run!
    int num_tokens = generate(&transformer, &tokenizer, &sampler, prompt, steps);

    print_time_elapsed(num_tokens);
    print_memory_usage();
    printf("\n");
